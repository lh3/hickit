...
int main(int argc, char *argv[])
{
	...
	struct hk_map *m = 0;
	struct hk_graph *g = 0;
	int c, is_pairs = 0, is_graph = 0, is_dedup = 0;

	hk_opt_init(&opt);
	while ((c = getopt(argc, argv, "pgDr:")) >= 0) {
		if (c == 'p') is_pairs = 1;
		else if (c == 'g') is_graph = 1;
		else if (c == 'D') is_dedup = 1;
	...
	if (is_pairs) {
		...
		hk_pair_print(stdout, m, n_pairs, pairs);
		free(pairs);
	} else {
