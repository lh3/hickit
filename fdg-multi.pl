...
my $conf = [["4m",   0.1, 20000, 0.01, 0.2, 0.0],
			["1m",   0.2,  5000, 0.02, 0.1, 0.0],
			["200k", 0.2,  1500, 0.02, 0.1, 0.0],
			["50k",  0.2,   750, 0.03, 0.1, 5.0],
			["20k",  0.2,   500, 0.03, 0.1, 5.0],
			["20k",  0.2,   500, 0.02, 0.1, 5.0]];
...
my ($prev, $next);
for (my $i = 0; $i < @$conf; ++$i) {
	my $cli_opts = qq/-S $opts{S} -b $conf->[$i][0] -k $conf->[$i][1] -n $conf->[$i][2] -e $conf->[$i][3] -f $conf->[$i][4] -d $conf->[$i][5]/;
