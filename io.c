...
static void hk_parse_pair(struct hk_pair *p, struct hk_sdict *d, int n_extra_cols, int *extra_cols, int n_fields, char **fields)
{
	...
	if (n_fields >= 7) {
		...
		for (i = 0; i < n_extra_cols; ++i) {
			...
			} else if (e == 10) { // exp_count
				p->_.ecnt.e = atof(fields[c]);
			} else if (e == 11) { // loop_q
				p->_.loop.q = atof(fields[c]);
			} else if (e == 12) { // loop_r
...
void hk_print_pair(FILE *fp, int flag, const struct hk_sdict *d, int32_t n_pairs, const struct hk_pair *pairs)
{
	...
	else if ((flag & 0x3800) == 0x3800) fprintf(fp, " %s %s %s", hk_pair_cols[11], hk_pair_cols[12], hk_pair_cols[13]);
	fputc('\n', fp);
	for (i = 0; i < n_pairs; ++i) {
		...
		else if (flag & 1<<10) fprintf(fp, "\t%.6f", p->_.ecnt.e);
		else if ((flag & 0x3800) == 0x3800) fprintf(fp, "\t%.1f\t%d\t%d", p->_.loop.q, p->_.loop.r, p->_.loop.n);
		fputc('\n', fp);
	}
}
